...
const createNewFilesPromises = arrStr.map((str, index) => {
  return fs.writeFile(`file${index + 1}.txt`, str);
});

// console.log(createNewFilesPromises);

Promise.all(createNewFilesPromises)
  .then((files) => {
    console.log(files);
