...
const greet = (temperature) => console.log(`Hi there! Curiosity here. Right now is ${temperature}ºF at Mars`);
...
const sendMarsTemperature = (success, err) => {
  setTimeout(() => {
    ...
    if (didOperationSucceed) {
      ...
      const fahrenheitTemperature = toFahrenheit(marsTemperature);
      success(fahrenheitTemperature);
    } else {
      const messageErr = 'Robot is busy';
      err(messageErr);
    }
  }, messageDelay());
};
